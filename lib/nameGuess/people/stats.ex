defmodule NameGuess.People.Stats do
  ...
  def get_most_guessed(space, division \\ nil, how_many \\ @number_of_stats) do
  ...
  def get_least_guessed(space, division \\ nil, how_many \\ @number_of_stats) do
    {query, parameters} = guessed_query("not_guessed", space, division, how_many)
    {:ok, result} = Repo.query(query, parameters)
    map_query_result(result)
  end

  defp guessed_query(field, space, division, how_many) do
    field_full = "ps." <> field
    ...
    {"""
     ...
     ((1.0 * #{field_full} / (ps.guessed + ps.not_guessed)) + $1::real * $1::real / (2 * (ps.guessed + ps.not_guessed)) - $1::real * SQRT(((1.0 * #{
       field_full
     } / (ps.guessed + ps.not_guessed)) * (1 - (1.0 * #{field_full} / (ps.guessed + ps.not_guessed))) + $1::real * $1::real / (4 * (ps.guessed + ps.not_guessed))) / (ps.guessed + ps.not_guessed))) / (1 + $1::real * $1::real / (ps.guessed + ps.not_guessed)) as final_score
    ...
    unless wrong_id == nil do
      ...
      {:ok, _updated} =
        Repo.insert(%WrongName{space: space, person: person, name: wrong.name, counter: 1},
          ...
          conflict_target: {:constraint, :wrong_name_pkey}
        )
    end
