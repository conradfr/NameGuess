defmodule NameGuess.DataSource.WikipediaPresidents do
  ...
  def import_image(president) do
    case HTTPoison.get(president.img_source, [], follow_redirect: true) do
  ...
  defp get_real_picture_path(presidents) do
    ...
    |> Enum.map(fn president ->
      case HTTPoison.get(@source_base <> president.picture, [], follow_redirect: true) do
        {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
          picture_url_full =
            body
  ...
  def get_presidents() do
    case HTTPoison.get(@source_url, [], follow_redirect: true) do
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        presidents_nodes =
          body
          |> Floki.parse_document!()
