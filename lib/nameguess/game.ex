defmodule NameGuess.Game do
  alias __MODULE__
  alias Timex
  alias NameGuess.{Picker, People, Space, Score, PictureRotate}
  ...
  def guess(%Game{state: :ok, countdown_timer: timer} = game, pick) do
  ...
  @doc """
    Should not happen
  """
  def guess(%Game{} = game, _pick) do
    game
  end

  @spec has_correct_answer(Game, DateTime) :: Game
  defp has_correct_answer(%Game{} = game, datetime_end) do
  ...
  defp has_win(%Game{} = game, datetime_end) do
    game = %{
      game
      | score: game.score + 1,
        state: :win_next,
  ...
  defp has_lost(%Game{} = game, pick, datetime_end) do
    {score_rank_this_month, score_rank_all_time} = get_score_ranks(game, datetime_end)
  ...
  def get_score_ranks(%Game{} = game, datetime_end) do
  ...
  def get_duration(%Game{} = game, datetime_end) when datetime_end != nil do
    DateTime.diff(datetime_end, game.started_at)
  end

  def get_duration(%Game{ended_at: ended_at} = game, _datetime_end) when ended_at == nil do
    {:ok, datetime_end} = DateTime.now("Etc/UTC")
    DateTime.diff(datetime_end, game.started_at)
  end

  def get_duration(%Game{} = game, _datetime_end) do
