defmodule NameGuess.Picker do
  ...
  def pick_next_round(%Space{} = space, previous_picks \\ [], round \\ 1) do
  ...
  def get_process_name(%Space{} = space) do
  ...
  defp save_picked_state(%Space{} = space, picked) do
  ...
  defp pick_unique(%Space{} = space, past_picks) do
  ...
  defp pick(%Space{} = _space, 0, _gender, already_picked) do
    already_picked
  end

  defp pick(%Space{} = space, how_many, gender, already_picked) do
    picked =
  ...
  defp pick_rand(%Space{} = space, how_many, gender) do
