defmodule NameGuess.Update do
  ...
  @spec process_source(String.t(), list(%{any => Person})) :: integer
  defp process_source(source, people) do
    ...
    Logger.info(
      "Update #{source}: added  #{really_added}/#{length(references_to_add)}, removed: #{length(ids_to_delete)}, updated: #{really_updated}/#{length(references_to_update)}"
    )
