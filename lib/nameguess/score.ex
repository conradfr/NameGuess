defmodule NameGuess.Score do
  ...
  alias NameGuess.{Cache, Game, Space, GameScore, HighScore}
  ...
  def get_space_duration(%Space{} = space) do
  ...
  def add_to_scores(%Game{} = game) do
  ...
  def count_score(%Space{} = space, score) do
    query =
  ...
  def is_high_score?(%Space{} = _space, 0, _duration) do
    false
  end

  def is_high_score?(%Space{} = space, score, duration) do
  ...
  def get_score_rank(%Space{} = space, score, duration) do
  ...
  def get_score_rank_this_month(%Space{} = space, score, duration) do
  ...
  defp high_score_rank_count(%Space{} = space, score, duration) do
  ...
  def get_highest_score(%Space{} = space) do
  ...
  def get_highest_score_this_month(%Space{} = space) do
  ...
  def get_high_scores(%Space{} = space, how_many \\ @number_of_high_scores) do
    query = get_high_scores_query(space.id, how_many)
    Repo.all(query)
  end

  @spec get_high_scores_this_month(Space, integer) :: list
  def get_high_scores_this_month(%Space{} = space, how_many \\ @number_of_high_scores) do
    query =
      from(q in get_high_scores_query(space.id, how_many),
        where:
          fragment(
            "date_trunc('month', ?)::date = date_trunc('month', now())::date",
            q.scored_at
          )
      )
  ...
  def add_to_best_scores(name, %Game{} = game) do
    duration = Game.get_duration(game)
